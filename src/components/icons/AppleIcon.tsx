import type { ComponentProps } from 'react';

export const AppleIcon = (props: ComponentProps<'svg'>) => (
  <svg height='1em' width='1em' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32' {...props}>
    <g fill='none'>
      <path
        d='M10.25 2H4.22a5.92 5.92 0 0 0 5.92 5.92h6.03C16.18 4.66 13.52 2 10.25 2'
        fill='url(#f622id4)'
      ></path>
      <g filter='url(#f622id8)'>
        <path
          d='M10.14 7.42A5.42 5.42 0 0 1 4.743 2.5h5.507a5.424 5.424 0 0 1 5.399 4.92z'
          fill='url(#f622id5)'
        ></path>
      </g>
      <g filter='url(#f622id9)'>
        <path
          d='M6.958 2.5h3.292a5.424 5.424 0 0 1 5.399 4.92c-2.275-3.93-3.167-3.779-8.691-4.92'
          fill='url(#f622id6)'
        ></path>
      </g>
      <path
        d='M18.09 7.38c-1.34.57-2.86.57-4.2 0c-4.2-1.77-9.04.26-10.72 4.51a16.72 16.72 0 0 0 0 12.29a9.227 9.227 0 0 0 11.06 5.49c1.15-.32 2.37-.32 3.52 0a9.203 9.203 0 0 0 11.06-5.49c1.56-3.95 1.56-8.34 0-12.29c-1.67-4.25-6.51-6.28-10.72-4.51'
        fill='url(#f622id0)'
      ></path>
      <path
        d='M18.09 7.38c-1.34.57-2.86.57-4.2 0c-4.2-1.77-9.04.26-10.72 4.51a16.72 16.72 0 0 0 0 12.29a9.227 9.227 0 0 0 11.06 5.49c1.15-.32 2.37-.32 3.52 0a9.203 9.203 0 0 0 11.06-5.49c1.56-3.95 1.56-8.34 0-12.29c-1.67-4.25-6.51-6.28-10.72-4.51'
        fill='url(#f622id1)'
      ></path>
      <path
        d='M18.09 7.38c-1.34.57-2.86.57-4.2 0c-4.2-1.77-9.04.26-10.72 4.51a16.72 16.72 0 0 0 0 12.29a9.227 9.227 0 0 0 11.06 5.49c1.15-.32 2.37-.32 3.52 0a9.203 9.203 0 0 0 11.06-5.49c1.56-3.95 1.56-8.34 0-12.29c-1.67-4.25-6.51-6.28-10.72-4.51'
        fill='url(#f622id7)'
      ></path>
      <path
        d='M18.09 7.38c-1.34.57-2.86.57-4.2 0c-4.2-1.77-9.04.26-10.72 4.51a16.72 16.72 0 0 0 0 12.29a9.227 9.227 0 0 0 11.06 5.49c1.15-.32 2.37-.32 3.52 0a9.203 9.203 0 0 0 11.06-5.49c1.56-3.95 1.56-8.34 0-12.29c-1.67-4.25-6.51-6.28-10.72-4.51'
        fill='url(#f622id2)'
      ></path>
      <path
        d='M18.09 7.38c-1.34.57-2.86.57-4.2 0c-4.2-1.77-9.04.26-10.72 4.51a16.72 16.72 0 0 0 0 12.29a9.227 9.227 0 0 0 11.06 5.49c1.15-.32 2.37-.32 3.52 0a9.203 9.203 0 0 0 11.06-5.49c1.56-3.95 1.56-8.34 0-12.29c-1.67-4.25-6.51-6.28-10.72-4.51'
        fill='url(#f622id3)'
      ></path>
      <defs>
        <radialGradient
          cx='0'
          cy='0'
          gradientUnits='userSpaceOnUse'
          id='f622id0'
          r='1'
          gradientTransform='rotate(-40.012 39.811 6.793)scale(28.2351)'
        >
          <stop stopColor='#689133'></stop>
          <stop offset='.528' stopColor='#A4D948'></stop>
          <stop offset='1' stopColor='#CCEE37'></stop>
        </radialGradient>
        <radialGradient
          cx='0'
          cy='0'
          gradientUnits='userSpaceOnUse'
          id='f622id1'
          r='1'
          gradientTransform='matrix(20.09575 14.29874 -17.18862 24.15726 4.746 6.74)'
        >
          <stop stopColor='#678F32'></stop>
          <stop offset='.39' stopColor='#96CB3A' stopOpacity='0'></stop>
        </radialGradient>
        <radialGradient
          cx='0'
          cy='0'
          gradientUnits='userSpaceOnUse'
          id='f622id2'
          r='1'
          gradientTransform='matrix(10.47528 20.3604 -24.4754 12.59242 12.196 14.348)'
        >
          <stop stopColor='#BFF341'></stop>
          <stop offset='.367' stopColor='#BFF341' stopOpacity='0'></stop>
        </radialGradient>
        <radialGradient
          cx='0'
          cy='0'
          gradientUnits='userSpaceOnUse'
          id='f622id3'
          r='1'
          gradientTransform='matrix(-14.45602 18.42518 -22.14903 -17.37767 24.181 14.258)'
        >
          <stop stopColor='#EEFF61'></stop>
          <stop offset='.386' stopColor='#EEFF61' stopOpacity='0'></stop>
        </radialGradient>
        <linearGradient
          gradientUnits='userSpaceOnUse'
          id='f622id4'
          x1='4.22'
          x2='16.17'
          y1='2'
          y2='7.92'
        >
          <stop stopColor='#77B725'></stop>
          <stop offset='1' stopColor='#A3DA32'></stop>
        </linearGradient>
        <linearGradient
          gradientUnits='userSpaceOnUse'
          id='f622id5'
          x1='4.22'
          x2='16.17'
          y1='2'
          y2='7.92'
        >
          <stop stopColor='#8FCD20'></stop>
          <stop offset='1' stopColor='#AEE137'></stop>
        </linearGradient>
        <linearGradient
          gradientUnits='userSpaceOnUse'
          id='f622id6'
          x1='6.958'
          x2='16.062'
          y1='2.5'
          y2='7'
        >
          <stop offset='.26' stopColor='#CDFF7A' stopOpacity='0'></stop>
          <stop offset='.74' stopColor='#CDFF7A' stopOpacity='.8'></stop>
          <stop offset='1' stopColor='#CDFF7A' stopOpacity='0'></stop>
        </linearGradient>
        <linearGradient
          gradientUnits='userSpaceOnUse'
          id='f622id7'
          x1='15.99'
          x2='15.99'
          y1='6.74'
          y2='30.016'
        >
          <stop offset='.583' stopColor='#A3CE5A' stopOpacity='0'></stop>
          <stop offset='1' stopColor='#A37CA3'></stop>
        </linearGradient>
        <filter
          filterUnits='userSpaceOnUse'
          height='5.92'
          id='f622id8'
          width='11.906'
          x='4.243'
          y='2'
          colorInterpolationFilters='sRGB'
        >
          <feFlood result='BackgroundImageFix' floodOpacity='0'></feFlood>
          <feBlend in='SourceGraphic' in2='BackgroundImageFix' result='shape'></feBlend>
          <feGaussianBlur
            result='effect1_foregroundBlur_18_30671'
            stdDeviation='.25'
          ></feGaussianBlur>
        </filter>
        <filter
          filterUnits='userSpaceOnUse'
          height='6.42'
          id='f622id9'
          width='10.191'
          x='6.208'
          y='1.75'
          colorInterpolationFilters='sRGB'
        >
          <feFlood result='BackgroundImageFix' floodOpacity='0'></feFlood>
          <feBlend in='SourceGraphic' in2='BackgroundImageFix' result='shape'></feBlend>
          <feGaussianBlur
            result='effect1_foregroundBlur_18_30671'
            stdDeviation='.375'
          ></feGaussianBlur>
        </filter>
      </defs>
    </g>
  </svg>
);
